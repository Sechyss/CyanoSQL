...
# %% Rich ideas to reorder the nodes
...
df_groups = pd.read_csv('/Users/u1866168/Documents/OneDrive - University of Warwick/'
                        'Experiments/Phylogenetic_congruency/Prokka_Run/Diverge_clustegram_analysis/Nodes.csv')
df_clade = pd.read_csv('/Users/u1866168/Documents/OneDrive - University of Warwick/'
                       'MHC_Scripts/SQL_Tables/SQL_Clade_table_prokka.csv')
...
for i in range(1, 17):
    ...
    plt.savefig('/Users/u1866168/Documents/OneDrive - University of Warwick/'
                'MHC_Scripts/Phylogenetic_congruency/Prokka_Run/Diverge_clustegram_analysis/NodeFigures/Node_' + str(
        i) + '.pdf'
                )
...
collection_df = collection_df.fillna(0)
res = sns.heatmap(collection_df, yticklabels=False, xticklabels=False, cbar=False)
plt.savefig('/Users/u1866168/Documents/OneDrive - University of Warwick/'
            'MHC_Scripts/Phylogenetic_congruency/Prokka_Run/Diverge_clustegram_analysis/NodeFigures/Node_all_heatmap.pdf'
            )

# %%
table_clade = pd.read_excel('/Users/u1866168/Documents/OneDrive - University of Warwick/'
                            'Experiments/Phylogenetic_congruency/Prokka_Run/Diverge_clustegram_analysis/'
                            'Nodes_clades.xlsx', sheet_name='Nodes_clades')
dict_total = {}
dict_hypnodeclade = {}
for column in table_clade:
    dict_hypnodeclade.update({str(column).rsplit('-', 1)[0]:
                                  int(table_clade[column].str.contains('hypothetical').value_counts()[True])})
    dict_total.update({str(column).rsplit('-', 1)[0]: int(table_clade[column].count())})
...
ax.bar(list(dict_total.keys()), list(dict_total.values()), align='center', color='navy', label='Total genes')
ax.bar(list(dict_total.keys()), list(dict_hypnodeclade.values()), align='center', color='gold',
       label='Hypotheticals')
ax.set_ylabel('Number of genes in nodes')
ax.set_xticklabels(list(dict_total.keys()), weight='bold')
ax.legend(loc='upper right', prop=font)
ax.yaxis.grid(True)
plt.tight_layout()
plt.savefig('/Users/u1866168/Documents/OneDrive - University of Warwick/'
            'Experiments/Phylogenetic_congruency/Prokka_Run/Diverge_clustegram_analysis/'
            'Node_genes_hypotheticals.pdf')
plt.show()

for element in dict_total:
    a = float((dict_hypnodeclade[element]/dict_total[element])*100)
    print('The proportion of hypothetical genes in ', element, 'is ',"{:.2f}".format(a), 'out of ', dict_total[element])
